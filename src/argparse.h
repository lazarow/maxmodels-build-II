...
#undef ARGPARSE_DEFINE_TRAITS
    ...
    public:
        ArgHolderBase(std::string fullname, char shortname, std::string help)
            : fullname_(std::move(fullname)), shortname_(shortname), help_(std::move(help)), required_(false) {}
    ...
    public:
        Parser()
            : holders_(), positionals_(), free_args_(std::nullopt), parse_global_args_(true), usage_string_(std::nullopt), exit_code_(std::nullopt) {}
