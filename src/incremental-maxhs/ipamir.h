...
extern "C"
{
     ...
     *
     * Required state: INPUT or OPTIMAL or SAT or UNSAT
     * State after: INPUT or OPTIMAL or SAT or UNSAT
     */
    IPAMIR_API int32_t ipamir_solve(void *solver);
    ...
    IPAMIR_API void ipamir_print_wcnf(void *solver);

    /**
     * Reset the search state while preserving all added hard and soft clauses.
     * After calling this function, the next call to 'ipamir_solve' will start
     * a fresh search from the beginning, but all previously added clauses remain.
     *
     * This is useful when you want to restart the search without incrementing
     * from the previous search state.
     *
     * Required state: INPUT or OPTIMAL or SAT or UNSAT
     * State after: INPUT
     */
    IPAMIR_API void ipamir_reset_search(void *solver);
